...
import { Upload, FileText, CheckCircle, XCircle, Loader, ArrowLeft, Image as ImageIcon, Sparkles } from 'lucide-react';
  ...
  return (
    <div className="w-full flex flex-col items-center py-12 px-4">
      ...
      <div className="w-full max-w-4xl text-center mb-10">
        <h1 className="text-3xl md:text-4xl font-bold tracking-tight mb-4">PDF to JPG</h1>
        <p className="text-lg text-muted-foreground">Extract images from your PDF securely in your browser. 100% private.</p>
      </div>
            ...
            {uploading && (
              <div className="space-y-2">
                <div className="flex items-center justify-between text-sm font-medium">
                  <span className="text-primary flex items-center"><Loader className="h-4 w-4 mr-2 animate-spin"/> Processing in browser...</span>
                  <span>{progress}%</span>
                </div>
            ...
            {downloadUrls.length > 0 && (
              <div className="p-6 bg-green-500/10 border border-green-500/20 rounded-xl flex flex-col space-y-4">
                <div className="flex flex-col items-center text-center">
                  ...
                  <h3 className="text-lg font-bold text-foreground">PDF Converted Successfully!</h3>
                  <p className="text-sm text-muted-foreground mt-1">Your images are ready to download.</p>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-4">
                  {downloadUrls.map((url, index) => (
                    <a
                      ...
                      download={`${file?.name.replace('.pdf', '')}_page_${index + 1}.jpg`}
                    >
