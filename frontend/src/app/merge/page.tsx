  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setProgress(50);
        // Now merge the files
        const mergeResponse = await fetch(`${apiBase}/merge`, {
          method: 'POST',
        ...
        if (mergeResponse.ok) {
          ...
          setDownloadUrl(`${apiBase}/download/${mergeData.downloadId}`);
          setProgress(100);
      ...
      setError('Error processing files. Please ensure the backend is running.');
    } finally {
      setUploading(false);
    }
  };
  ...
  return (
    <div className="w-full flex flex-col items-center py-12 px-4">
      ...
      <div className="w-full max-w-4xl text-center mb-10">
        <h1 className="text-3xl md:text-4xl font-bold tracking-tight mb-4">Merge PDF</h1>
        <p className="text-lg text-muted-foreground">Combine multiple PDF files into one single document instantly.</p>
      </div>
          ...
          <div className="space-y-8">
            ...
            {!downloadUrl && (
              ...
              >
                {uploading ? (
                  ...
                  <span>Merge {files.length > 0 ? files.length : ''} PDFs</span>
                )}
              </button>
            )}
            ...
            {downloadUrl && (
              <div className="p-6 bg-green-500/10 border border-green-500/20 rounded-xl flex flex-col items-center text-center space-y-4">
                ...
                <div>
                  <h3 className="text-lg font-bold text-foreground">PDFs Merged Successfully!</h3>
                  <p className="text-sm text-muted-foreground mt-1">Your new document is ready to download.</p>
                </div>
                <a
                  href={downloadUrl}
