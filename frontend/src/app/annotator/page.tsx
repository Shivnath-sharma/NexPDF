...
import {
  ArrowLeft, Highlighter, PenLine, Type, Eraser,
...
export default function PDFAnnotator() {
  ...
  useEffect(() => {
    if (textPos && textInputRef.current) setTimeout(() => textInputRef.current?.focus(), 50);
  }, [textPos]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      for (let i = 0; i < pages.length; i++) {
        const dataUrl = annotationsRef.current.get(i + 1);
        if (!dataUrl) continue;
        const img = new Image();
        ...
        if (!Array.from(px).some((v, idx) => idx % 4 === 3 && v > 0)) continue;
    ...
    <div className="w-full flex flex-col items-center py-8 px-4 min-h-screen">
      ...
      <div className="w-full max-w-7xl flex items-center justify-between mb-6">
        ...
        <div className="text-center">
          ...
          {file && <p className="text-xs text-muted-foreground mt-0.5 truncate max-w-[180px] sm:max-w-xs">{file.name}</p>}
        </div>
      ...
      {!file && !loading && (
        <div className="w-full max-w-3xl">
          ...
          <div
            {...getRootProps()}
            className={`border-2 border-dashed rounded-2xl p-10 sm:p-14 text-center cursor-pointer transition-all duration-300 ${isDragActive ? 'border-amber-500 bg-amber-500/5 scale-[1.01]' : 'border-border bg-card hover:border-amber-500/50'
              }`}
          >
            <input {...getInputProps()} />
      ...
      {file && !loading && (
        <div className="w-full max-w-7xl flex flex-col gap-5">
          ...
          <div className="flex lg:hidden gap-2 overflow-x-auto pb-1">
            {([
              { id: 'pen' as Tool, label: 'Pen', icon: PenLine },
              { id: 'highlight' as Tool, label: 'Highlight', icon: Highlighter },
              { id: 'text' as Tool, label: 'Text', icon: Type },
              { id: 'eraser' as Tool, label: 'Eraser', icon: Eraser },
            ]).map(({ id, label, icon: Icon }) => (
              <button key={id} onClick={() => setTool(id)}
                className={`shrink-0 flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold transition-all ${tool === id ? 'bg-amber-500 text-white shadow-md' : 'bg-card border border-border text-muted-foreground'
                  }`}
              >
            ...
            {(tool === 'highlight' ? HIGHLIGHT_COLORS : COLORS).map((c) => (
              <button key={c}
                ...
                className={`shrink-0 h-8 w-8 rounded-full border-2 transition-transform hover:scale-110 ${activeColor === c ? 'border-foreground scale-110' : 'border-transparent'}`}
                style={{ backgroundColor: c }}
              />
            ))}
            <div className="shrink-0 w-px bg-border mx-1" />
            <button onClick={clearPage} className="shrink-0 flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold bg-card border border-border text-destructive">
              <Trash2 className="h-4 w-4" /> Clear
            </button>
            {downloadUrl ? (
              <a href={downloadUrl} download={`annotated_${file.name}`} className="shrink-0 flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold bg-green-600 text-white">
                <CheckCircle className="h-4 w-4" /> Download
              </a>
            ) : (
              <button onClick={handleSave} disabled={saving} className="shrink-0 flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold bg-amber-500 text-white disabled:opacity-50">
          ...
          {/* Main editor: sidebar + canvas */}
          <div className="flex flex-col lg:flex-row gap-5">

            {/* Desktop sidebar */}
            <div className="hidden lg:flex lg:w-64 shrink-0 flex-col gap-4">
              ...
              <div className="bg-card border border-border rounded-2xl p-4 space-y-2 shadow-sm">
                ...
                {([
                  { id: 'pen' as Tool, label: 'Pen', icon: PenLine, desc: 'Freehand draw' },
                  { id: 'highlight' as Tool, label: 'Highlight', icon: Highlighter, desc: 'Drag to highlight' },
                  { id: 'text' as Tool, label: 'Text', icon: Type, desc: 'Click to type' },
                  { id: 'eraser' as Tool, label: 'Eraser', icon: Eraser, desc: 'Erase annotations' },
                ]).map(({ id, label, icon: Icon, desc }) => (
                  <button key={id} onClick={() => setTool(id)}
                    className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all text-left ${tool === id ? 'bg-amber-500 text-white shadow-md shadow-amber-500/30' : 'hover:bg-muted text-muted-foreground hover:text-foreground'
                      }`}
                  >
                ...
                <div className="bg-card border border-border rounded-2xl p-4 shadow-sm">
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {(tool === 'highlight' ? HIGHLIGHT_COLORS : COLORS).map((c) => (
                      <button key={c}
                        onClick={() => tool === 'highlight' ? setHighlightColor(c) : setColor(c)}
                        className={`h-8 w-full rounded-lg border-2 transition-transform hover:scale-110 ${activeColor === c ? 'border-foreground scale-110' : 'border-transparent'}`}
                        style={{ backgroundColor: c }}
                      />
                    ))}
                  </div>
              ...
              <div className="bg-card border border-border rounded-2xl p-4 shadow-sm">
                ...
                <button onClick={clearPage} className="w-full flex items-center gap-2 px-3 py-2.5 rounded-xl text-sm font-medium text-destructive hover:bg-destructive/10 transition-colors">
            ...
            <div className="flex-1 flex flex-col gap-3">
              <div className="bg-muted/30 border border-border rounded-2xl overflow-hidden flex items-center justify-center p-4 relative">
                <div className="relative shadow-2xl" style={{ maxWidth: '100%' }}>
                  <canvas ref={pdfCanvasRef} className="block rounded-lg" style={{ maxWidth: '100%', display: 'block' }} />
                  <canvas
                    ref={annCanvasRef}
                    ...
                    style={{
                      ...
                      touchAction: 'none',
                      cursor: tool === 'pen' ? 'crosshair' : tool === 'eraser' ? 'cell' : tool === 'text' ? 'text' : 'crosshair',
                    }}
              ...
              <div className="flex items-center justify-center gap-4 bg-card border border-border rounded-xl px-5 py-3 shadow-sm">
                ...
                <button onClick={() => goToPage(Math.min(numPages, currentPage + 1))} disabled={currentPage >= numPages}
                  className="p-2 rounded-lg hover:bg-muted disabled:opacity-30 transition-colors">
              ...
              <p className="text-center text-xs text-muted-foreground">
                {tool === 'text' ? '💬 Click on the page to place a text label, then press Enter to confirm'
                  : tool === 'highlight' ? '🖍️ Click and drag to highlight an area'
                    : tool === 'eraser' ? '🧹 Click and drag to erase annotations'
